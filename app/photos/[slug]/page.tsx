...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (adminAlbum) {
    const description = adminAlbum.excerpt || `Photo album: ${adminAlbum.title}`
    const ogImages = adminAlbum.cover
      ? [{ url: resolveImage(adminAlbum.cover) ?? '' }]
      : []
    return {
      title: adminAlbum.title,
      description,
      openGraph: { title: adminAlbum.title, description, images: ogImages },
    }
  }

  const activity = activities.find(a => a.slug === slug)
  if (activity) {
    const description = activity.excerpt || `Photo album: ${activity.title}`
    return {
      title: activity.title,
      description,
      openGraph: { title: activity.title, description },
    }
  }

  return { title: 'Photo Album', description: 'Photography by Vikhyat Kumar Srivastava.' }
}
